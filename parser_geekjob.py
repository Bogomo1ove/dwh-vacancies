...
def get_vacancies_links(page_url: str) -> List:
    ...
    links_list = [
        ...
        for link in soup.find("ul", id="serplist", class_="collection serp-list")
...
def parse_page(link: str) -> Dict:
    ...
    common_block = (
        ...
        .find("section", class_="col s12 m12 main").find("header")
    )
    ...
    description = (
        ...
        .find("div", id="vacancy-description").text
    )
    return dict(title=vacancy_name,
...
def get_vacancies_geekjob(url):
    vacancies_links = [
            page_link for page_number in range(1, get_pages_count(url) + 1)
            for page_link in get_vacancies_links(f"{URL}/vacancies/{page_number}")
            ]
    parsed_vacancies = [parse_page(link) for link in vacancies_links]
    with open(f"{datetime.today().isoformat()}.json", "w", encoding="utf-8") as file:
